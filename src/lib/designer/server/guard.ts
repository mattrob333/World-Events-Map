...
/**
 * Warm-instance guard on paid AI calls. When it trips, routes fall back to
 * the free on-device composer instead of failing. It is per instance, not a
 * durable budget; a durable spend cap is required before public launch.
 */
export function consumeAiCall(request: Request, now = Date.now()): boolean {
  ...
  if (bucket.count >= AI_CALLS_PER_WINDOW) {
