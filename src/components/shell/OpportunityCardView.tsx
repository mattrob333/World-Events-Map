  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <span className="label-sm text-brass">
          {OPPORTUNITY_KIND_LABEL[offer.kind]}
          {offer.sample ? <span className="ml-2 rounded-[2px] border border-ink/20 px-1 text-ink-muted">Sample</span> : null}
        </span>
      ...
      <div>
        ...
        <p className="mt-1 text-[12px] text-ink-muted">{offer.subtitle} · {offerWindowLabel(offer)}</p>
      </div>
      <p className="text-[13px] leading-5 text-ink-muted">{offer.body}</p>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-[12px] text-ink">{offer.sample ? `Sample provider · ${offer.providerName}` : offer.providerName}</p>
          <p className="text-[11px] text-ink-muted">{offer.priceLabel}</p>
        </div>
        <Link href={offer.href} className={buttonClassName({ variant: 'brass' })}>
