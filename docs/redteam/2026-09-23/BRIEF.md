...
- Never fabricate screenshots, browser results, console output, Jev output, or user behaviour. Jev by TypeSafe and treg are NOT available to testers in this environment: mark Jev judgments `JEV NOT EXECUTED` and give your own rubric answer separately labelled as analyst judgment; mark external checks `EXTERNAL VALIDATION NOT EXECUTED`.
